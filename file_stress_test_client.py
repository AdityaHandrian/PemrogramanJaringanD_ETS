...
class StressTestClient:
    ...
    def send_command(self, command_str=""):
        ...
        sock.settimeout(1800)  # 30 minutes timeout for large files
        try:
            ...
            while total_sent < len(command_bytes):
                chunk_size = min(65536, len(command_bytes) - total_sent)  # 64KB chunks
                sent = sock.send(command_bytes[total_sent:total_sent + chunk_size])
                total_sent += sent
                ...
                for operation in operations:
                    for file_size in file_sizes:
                        for client_pool_size in client_pool_sizes:
                            logging.info(f"Running test: Operation={operation}, FileSize={file_size}MB, ClientPool={client_pool_size}, ServerPool={server_pool_size}, ExecutorType={executor_type}")
